...
async function verifyOTP() {
    ...
    try {
        ...
        if (data.success && data.token) {
            ...
            // Store auth data
            try {
                localStorage.setItem("ruralassist_token", data.token);
                localStorage.setItem("ruralassist_logged_in", "true");
                localStorage.setItem("user_email", email);
                
                // Ask for name if not set
                if (!localStorage.getItem("ruralassist_name")) {
                    const name = prompt("Welcome! What's your name? (optional):");
                    if (name && name.trim()) {
                        localStorage.setItem("ruralassist_name", name.trim());
                    }
                }
            } catch (storageErr) {
            ...
            const target = localStorage.getItem("login_redirect_target");
            if (target) {
                try { localStorage.removeItem("login_redirect_target"); } catch {}
