...
def send_otp_email(receiver_email: str, otp: str, user_name: str = None) -> Tuple[int, str]:
    ...
    # Validate API key
    if not BREVO_API_KEY or BREVO_API_KEY == "your_actual_brevo_api_key_here":
        return 500, "❌ Email service not configured. Please set BREVO_API_KEY in .env file."
...
def send_welcome_email(receiver_email: str, user_name: str = None) -> Tuple[int, str]:
    ...
    if not BREVO_API_KEY or BREVO_API_KEY == "your_actual_brevo_api_key_here":
        return 500, "Email service not configured"
