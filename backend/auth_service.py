...
# ----- CONFIG -----
JWT_SECRET = os.getenv("JWT_SECRET", "RURALASSIST_SECRET_KEY_2025_SECURE")
...
def send_email_otp_endpoint(req: SendOtpRequest):
    ...
    if EMAIL_ENABLED:
        try:
            ...
            if status_code == 201:
                ...
                # Email send failed, but still allow console fallback
                print(f"⚠️ Email send failed for {email}: {message}")
                print(f"📧 OTP for {email}: {otp} (Console fallback)")
                return AuthResponse(
                    success=True,
                    message=f"⚠️ Email service issue. Check console for OTP (Development mode)",
                    token=None
                )
            ...
            return AuthResponse(
                success=True,
                message=f"⚠️ Email service unavailable. Check console for OTP (Development mode)",
                token=None
            )
    else:
...
def verify_email_otp_endpoint(req: VerifyOtpRequest):
